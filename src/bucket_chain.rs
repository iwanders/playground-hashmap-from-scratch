...
pub trait BucketKeyReq: Hash + Eq + Default {}
impl<T: Hash + Eq + Default> BucketKeyReq for T {}

const DEFAULT_BUCKET_LOAD_FACTOR_MAX: f64 = 1.0;
const DEFAULT_BUCKET_RESIZE_LOAD_FACTOR: f64 = 0.5;

#[derive(Debug)]
pub struct BucketInChainHashMap<K: BucketKeyReq, V: Default> {
    entries: usize,
    load_factor_max: f64,
    resize_load_factor: f64,
    buckets: Vec<tinyvec::TinyVec<[(K, V); 1]>>,
}
impl<K: BucketKeyReq, V: Default> Default for BucketInChainHashMap<K, V> {
    fn default() -> Self {
        Self {
            entries: 0,
...
impl<K: BucketKeyReq + Clone, V: Clone + Default> Clone for BucketInChainHashMap<K, V> {
    fn clone(&self) -> Self {
        Self {
...
impl<K: BucketKeyReq, V: Default> BucketInChainHashMap<K, V> {
...
mod test {
    ...
    fn test_bucket_seperate_chain() {
        let mut h = BucketInChainHashMap::<u64, u64>::new();
        h.insert(300, 3);
        h.insert(500, 8);
